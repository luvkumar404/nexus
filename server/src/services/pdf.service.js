...
export async function buildAuditPdf(audit) {
  ...
  (audit.categories || []).forEach((category) => {
    doc.fontSize(9).text(`${category.name}: ${category.score ?? 'n/a'}/100 (${category.passed} pass, ${category.warnings} warn, ${category.failed} fail)`);
  });
  doc.moveDown().fontSize(14).text('Top Priority Issues');
  failing.slice(0, 10).forEach((item, index) => {
    doc.fontSize(10).text(`${index + 1}. [${item.severity.toUpperCase()}] ${item.ruleName}`);
    doc.fontSize(9).fillColor('#555').text(`${item.affectedUrl || audit.url} - ${item.recommendation || ''}`).fillColor('#000');
  });
  ...
  Object.entries(audit.headingStructure || {}).forEach(([level, values]) => doc.fontSize(9).text(`${level.toUpperCase()}: ${(values || []).join(' | ') || 'Not found'}`));
  ...
  rules.forEach((rule) => {
    doc.fontSize(8).text(`${rule.ruleId} [${rule.status}] ${rule.ruleName}: ${rule.message}`);
  });
  doc.end();
  return done;
}
